...
**Срез изменений:** production остаётся на `0661777`, включая маскирование текста перед LLM и идемпотентность команд оператора; SHA следующего финального деплоя ещё не зафиксирован. Два прежних локальных запуска `pnpm launch` подтвердили проверку набора **40 сценариев / 43 исходных слота / 31 действие**, production-build и старт на порту **13301**. Основной код нового пакета собирался до последних изменений; финальный пакет и мобильный интерфейс ещё проходят проверку. Рабочее дерево содержит калибровку VAD, повторно используемый аудиоэлемент, измерение реального TTS, интерфейс с чёрной голосовой сценой и RU/KK/TR. Пользователь сообщил о проблемах живого голоса; конкретное описание ещё ожидается. Ни прежние сборки, ни наличие этих изменений не означают завершённую человеческую приёмку или новые unit/API-тесты.
...
| Опубликованная версия `0661777` | [Production URL](https://dir-echoes-voice-router.vercel.app): Ready, вход, 40 сценариев, супервизорский API и сохранённая история | Следующий финальный SHA ещё не зафиксирован; изменения голоса и UI рабочего дерева не объявляются опубликованными |
...
Приоритет следующей работы — завершить мобильную проверку, собрать и опубликовать финальный пакет с точным SHA, получить описание замеченных пользователем голосовых проблем и выполнить приёмку сокомандником на живом микрофоне. Новая настройка VAD, подготовка воспроизведения и реакция частиц не объявляются доказательством устранения ещё не описанной проблемы или достижения 1,5 секунды.

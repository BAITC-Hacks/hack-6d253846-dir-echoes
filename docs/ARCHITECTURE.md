...
**Срез изменений:** опубликована версия `0661777`, включая маскирование текста перед LLM и идемпотентность команд оператора. Серверные исправления `cb0c595` запушены 23 сентября в 15:28, но здесь не объявляются опубликованными. Новый голосовой интерфейс описан по рабочему дереву: его сборка, деплой и приёмка сокомандником с живым микрофоном на этом срезе ещё не подтверждены.
...
| Индикатор голоса | 640 частиц Canvas2D, реальный уровень микрофона и цвета состояний; [voice-particles.tsx](../src/components/voice-particles.tsx) |
...
[Сфера частиц](../src/components/voice-particles.tsx) меняет форму по фактическому `micLevel` только в состояниях listening/speaking. Цвет показывает состояние: серый — ожидание, бирюзовый — прослушивание, зелёный — речь клиента, янтарный — обработка, синий — ответ, красный — ошибка. Анимация ответа показывает статус и не измеряет амплитуду TTS. Canvas учитывает уменьшение движения, DPR до 2 и останавливается в скрытой вкладке; доступный текст состояния расположен рядом.
...
В новом интерфейсе trace по умолчанию свёрнут и открывается кнопкой «Логика ответа». Это меняет отображение, но не сбор и сохранение трассировки.
...
| Опубликованная версия `0661777` | [Production URL](https://dir-echoes-voice-router.vercel.app): Ready, вход, 40 сценариев, супервизорский API и сохранённая история | Маскирование и идемпотентность оператора включены; серверный checkpoint `cb0c595` и новый голосовой UI требуют отдельной публикации и приёмки |

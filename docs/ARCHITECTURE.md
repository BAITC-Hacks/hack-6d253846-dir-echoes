...
**Подтверждённый production:** `5aac0b52419687cd58fc4c851bb30eae79a07820`, Vercel `dpl_3KKUhDrM3dGtMq1no3MBNuygwjtv` — Ready. [Основной адрес](https://dir-echoes-voice-router.vercel.app) и [неизменяемый адрес этого деплоя](https://dir-echoes-voice-router-6aeb0oeyz-shadowocc.vercel.app) ведут к опубликованному пакету с калибровкой VAD, постоянным аудиоэлементом, измерением TTS и новым интерфейсом. Подтверждены `pnpm launch`, production-build/TypeScript, Vercel build, `/api/health` со статусом `ok`, загрузка нового экрана и сохранённой истории собственного разговора. В браузерных размерах 390×844, 390×500 и 844×390 горизонтального переполнения нет. Это не физическая проверка экранной клавиатуры, Safari или живого аудио. Описание сообщённой пользователем голосовой проблемы ещё ожидается; человеческая приёмка не закрыта. Следующие изменения частиц рабочего дерева в этот подтверждённый деплой не включены.
...
| Воспроизведение и измерение | Один аудиоэлемент, подготовленный в пользовательском жесте, потоковый MSE / Blob; RMS фактического ответа около 15 Гц; [audio-playback.ts](../src/lib/audio-playback.ts), [use-playback-meter.ts](../src/lib/use-playback-meter.ts) |
| Индикатор голоса | 3400 частиц Canvas2D, реальный уровень микрофона и TTS; 180 независимых фоновых звёзд; [voice-particles.tsx](../src/components/voice-particles.tsx) |
...
[use-playback-meter.ts](../src/lib/use-playback-meter.ts) измеряет реальный PCM ответа через Web Audio примерно **15 раз в секунду**. Подготовка вызывает `AudioContext.resume()` в том же пользовательском жесте; `MediaElementSource` создаётся только после успешного возобновления. Прямой путь к динамикам отделён от анализатора. Пауза, окончание воспроизведения и скрытая вкладка обнуляют уровень; скрытие не отключает звуковой граф. При размонтировании элементы останавливаются и контекст закрывается. Политика браузера всё ещё может потребовать новый жест после прерывания аудиоконтекста; измерение не доказывает физическую слышимость на устройстве.

[Органичная форма из 3400 частиц](../src/components/voice-particles.tsx) получает `micLevel` во время прослушивания/речи клиента и фактический уровень воспроизведения во время ответа. Прослушивание и речь клиента — зелёные, обработка — янтарная, ответ AI — оранжевый, ошибка — красная; ожидание нейтральное. **180 фоновых звёзд** двигаются независимо и не являются измерителем аудио. Canvas учитывает уменьшение движения, DPR до 2 и останавливается в скрытой вкладке; доступный текст состояния расположен рядом. Реакция на TTS теперь связана с сигналом, хотя сама форма остаётся визуальным представлением.
...
| Текущий production `5aac0b5` | Ready; `pnpm launch` / TypeScript / Vercel build; health `ok`; новый экран и прежняя история собственного разговора; размеры 390×844, 390×500, 844×390 без горизонтального переполнения | Нет новой человеческой проверки аудио, Safari и клавиатуры ОС; следующие изменения частиц ещё не опубликованы |
...
Приоритет следующей работы — получить описание замеченных пользователем голосовых проблем, выполнить приёмку сокомандником на живом микрофоне и проверить реальную мобильную клавиатуру/Safari. Для следующих изменений требуется новый отдельный deployment с SHA. Публикация `5aac0b5`, настройка VAD и реакция частиц не объявляются доказательством устранения ещё не описанной проблемы или достижения 1,5 секунды.

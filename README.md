...
> **Публикация:** версия `cced122` доступна на Vercel (Ready); подтверждены вход, 40 сценариев, супервизорская статистика и история после перезагрузки. Одно быстрое продолжение заняло 3 мс на выбор маршрута и 107 мс на сервере; первое событие воспроизведения — через 2,72 с от отправки текста. Это не замер живой речи и не общая задержка LLM. Живой микрофон RU/KK/mixed и полная приёмка действий остаются открытыми. [Подробности проверок](docs/CHECKPOINTS.md).

...
> **Публикация:** production **`5aac0b5`** доступен на Vercel (Ready): [основной адрес](https://dir-echoes-voice-router.vercel.app) · [неизменяемый адрес этого деплоя](https://dir-echoes-voice-router-6aeb0oeyz-shadowocc.vercel.app). Подтверждены `pnpm launch` со сборкой и TypeScript, Vercel build, `/api/health` со статусом `ok`, загрузка нового экрана и прежней истории собственного разговора в браузере. При размерах 390×844, 390×500 и 844×390 горизонтального переполнения не обнаружено. Физическая экранная клавиатура, Safari и живое аудио этой проверкой не покрыты. Пользователь сообщил о проблемах голоса; подробное описание ожидается. [Матрица ТЗ](docs/ACCEPTANCE.md) · [Точный SHA и deployment](docs/CHECKPOINTS.md).
...
Текущий подтверждённый production: `5aac0b52419687cd58fc4c851bb30eae79a07820`, deployment `dpl_3KKUhDrM3dGtMq1no3MBNuygwjtv` (Ready). Последующие правки частиц в рабочем дереве не входят в подтверждение этого деплоя. Приведённые результаты скорости и качества относятся к указанным версиям; цели 500 мс для router и 1,5 с до голосового ответа не объявляются достигнутыми.

...
**От свободной речи к проверяемому действию контакт-центра.** Решение команды DIR ECHOES для кейса Voice Router на HackAlem AI: русский, казахский и смешанная речь, выбор сценария через LLM, исполнение по правилам и рабочее место супервизора.

[Приложение](https://dir-echoes-voice-router.vercel.app) · [Командный репозиторий](https://github.com/BAITC-Hacks/hack-6d253846-dir-echoes) · [Архитектура](docs/ARCHITECTURE.md) · [План из трёх частей](docs/PLAN.md) · [Эксплуатация](docs/OPERATIONS.md) · [Сдача решения](docs/SUBMISSION.md)

> **Публикация:** первый деплой доступен на Vercel; подтверждены HTTP 200, вход, загрузка 40 сценариев и сохранённая история. Расширенные подтверждения, супервизия, быстрые продолжения и потоковое аудио ожидают финального деплоя и браузерной проверки. Голосовой цикл с живым микрофоном пока не подтверждён.

## Что реализовано

- **Разговор:** текст, запись микрофона через MediaRecorder, серверное распознавание, ответ и озвучивание. Текст позволяет продолжить при недоступном микрофоне.
- **Маршрутизация:** 40 сценариев и 3 системных намерения; структурированный ответ LLM содержит язык, сценарии, уверенность, альтернативы и параметры. Для смешанной речи отдельно выбирается язык ответа. Точное «да / иә / нет / жоқ» к сохранённой операции и проверенное значение одного запрошенного поля продолжают текущий сценарий без нового вызова LLM.
...
- **Рабочее место:** история и экспорт разговоров, каталог, трассировка, действия и измерения времени. Супервизор принимает обращение, отвечает и закрывает его; оценивает выбранный сценарий и сохраняет версии описаний и примеров каталога.
...
| Распознавание | OpenAI `gpt-transcribe`, параметры RU/KK; модель настраивается |
...
| `LOCAL_DATABASE_PATH` | Необязательный каталог PGlite; только для разработки |
...
В браузерных ответах маскируются телефоны, ИИН и email. Доступ к разговору и роль супервизора проверяются сервером. Общие коды ролей подходят для закрытого мероприятия; полноценные учётные записи, MFA и управление персоналом требуют отдельного внедрения.

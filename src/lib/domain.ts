...
async function executeDomainTurn(input: ExecuteInput): Promise<ExecuteOutput> {
  ...
  if (primary?.scenarioId === "SYS_OUT_OF_SCOPE" && primary.confidence >= 0.75) { output.reply = text("По этому вопросу у меня нет данных. Я могу помочь со страхованием Saqta: например, проверить полис или рассчитать стоимость страховки.", "Бұл сұрақ бойынша менде дерек жоқ. Saqta сақтандыруы бойынша көмектесемін: мысалы, полисті тексеру немесе сақтандыру құнын есептеу.", undefined, "Bu konuda bilgim yok. Saqta sigortasıyla ilgili yardımcı olabilirim: örneğin poliçe kontrolü veya sigorta bedeli hesabı."); return output; }

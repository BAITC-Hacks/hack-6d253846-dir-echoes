...
type SignedToken = (Viewer & { purpose: "session" }) | { id: string; exp: number; purpose: "browser_identity" };
...
function encode(value: SignedToken) { const payload = Buffer.from(JSON.stringify(value)).toString("base64url"); return payload + "." + signature(payload); }
function decode(value: string, purpose: SignedToken["purpose"]): SignedToken | null {
  try { const [payload, sig, extra] = value.split("."); if (!payload || !sig || extra || !safeEqual(signature(payload),sig)) return null;
    const decoded = JSON.parse(Buffer.from(payload,"base64url").toString()) as SignedToken;
    if(decoded.purpose!==purpose || typeof decoded.id!=="string" || !Number.isFinite(decoded.exp) || decoded.exp<=Date.now()) return null;
    if(decoded.purpose==="session" && !["participant","supervisor"].includes(decoded.role)) return null;
    return decoded;
  } catch { return null; }
}
export async function viewer(): Promise<Viewer | null> { const token=decode((await cookies()).get(cookieName)?.value || "","session"); return token?.purpose==="session"?token:null; }
...
export async function signIn(code: string, role: Role) {
  ...
  const jar = await cookies(); const existing = decode(jar.get("echoes_browser")?.value || "","browser_identity");
  ...
  jar.set(cookieName,encode({...value,purpose:"session"}),{...options,maxAge:12*60*60});
  jar.set("echoes_browser",encode({id:value.id,exp:Date.now()+30*24*60*60*1000,purpose:"browser_identity"}),{...options,maxAge:30*24*60*60});
  return value;
}

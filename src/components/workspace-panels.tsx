...
import { ArrowDownToLine, ArrowRight, AudioLines, BookOpen, Check, ChevronDown, ChevronRight, CircleCheck, Clock3, GitBranch, Headphones, History, Layers3, MessageSquare, Search, ShieldCheck, SlidersHorizontal, Workflow } from "lucide-react";
import type { ActionResult, DialogueState, Handoff, Json, Scenario, Session, SessionDetail, Turn } from "@/lib/types";
import { api, dateTime, duration, EmptyState, ErrorNotice, languageLabel, readableError, sessionStatus, Spinner, time } from "./workspace-ui";
...
function scenarioName(catalog: Scenario[], id: string) { return catalog.find(s => s.scenario_id === id)?.name ?? id; }
...
export function TracePanel({ detail, catalog, selectedTurnId, onSelectTurn }: { detail: SessionDetail | null; catalog: Scenario[]; selectedTurnId: string | null; onSelectTurn: (id: string) => void }) {
    ...
    <div className="trace-scroll">
      {tab === "context" ? <>
        <section className="trace-section"><div className="trace-section-heading"><Layers3 size={15} /><h3>Состояние разговора</h3></div>{state ? <>
          <dl className="slot-list"><div><dt>Статус</dt><dd>{sessionStatus(state.status)}</dd></div><div><dt>Язык</dt><dd>{languageLabel(state.language)}</dd></div><div><dt>Текущий сценарий</dt><dd>{active?.name ?? "Не выбран"}</dd></div></dl>
        ...
        <section className="trace-section first-trace-section"><div className="trace-section-heading"><GitBranch size={15} /><h3>Выбранный маршрут</h3><span className="micro-badge">{trace.source === "llm" ? "LLM" : trace.source === "operator" ? "Оператор" : "Подтверждение"}</span></div>
          ...
          {trace.scenarios.length > 0 && <p className="trace-reason">{trace.reason}</p>}
          <p className="tiny muted">Проценты — оценка выбора модели.</p>
...
export function OperatorsView({ handoffs, onChanged, onOpen }: { handoffs: Handoff[]; onChanged: () => Promise<unknown>; onOpen: (id: string) => void }) {
  ...
  const filtered = handoffs.filter(h => filter === "all" || (filter === "open" ? h.status !== "closed" : h.status === filter));
  const selected = handoffs.find(h => h.id === selectedId);

  useEffect(() => {
    setMessage(""); setError(null); setDetail(null);
    if (!selected) return;
    let current = true;
    setLoading(true);
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        if (selected && selected.status !== "closed") {
          ...
          if (!stopped) setDetail(updated);
        }
      } catch { /* Keep the current queue usable during a temporary polling failure. */ }
    }, 5000);
    return () => { stopped = true; clearInterval(poll); };
  }, [onChanged, selected?.id, selected?.sessionId, selected?.status, busy]);

  async function update(status: "active" | "closed", withMessage: boolean) {
    if (!selected || busy) return;
    setBusy(true); setError(null);
    try {
      await api(`/api/handoffs/${selected.id}`, { method: "PATCH", body: JSON.stringify({ status, ...(withMessage ? { message: message.trim() } : {}) }) });
      setMessage("");
      await onChanged();
      setDetail(await api<SessionDetail>(`/api/sessions/${selected.sessionId}`));
    } catch (err) { setError(readableError(err)); } finally { setBusy(false); }
  }

  return <section className="collection-page"><div className="collection-toolbar"><div className="segmented-control"><button className={filter === "open" ? "active" : ""} onClick={() => setFilter("open")}>Открытые <span>{handoffs.filter(h => h.status !== "closed").length}</span></button><button className={filter === "closed" ? "active" : ""} onClick={() => setFilter("closed")}>Завершённые</button><button className={filter === "all" ? "active" : ""} onClick={() => setFilter("all")}>Все</button></div><span className="result-count">{filtered.length} обращений</span></div>
    {!filtered.length ? <EmptyState icon={<Headphones size={27} />} title="В очереди нет обращений" description="Здесь появятся разговоры, для которых требуется участие оператора, вместе с контекстом и причиной передачи." /> : <div className="operator-layout"><div className="handoff-list">{filtered.map(h => <button className={`handoff-card ${h.id === selectedId ? "selected" : ""}`} key={h.id} onClick={() => setSelectedId(h.id)}><div><span className={`status-badge status-${h.status}`}><i />{h.status === "waiting" ? "Ожидает" : h.status === "active" ? "В работе" : "Закрыто"}</span><time>{time(h.createdAt)}</time></div><h3>{h.queue}</h3><p>{h.summary || h.reason}</p><span className="handoff-reason">{h.reason}</span></button>)}</div><div className="operator-detail">{!selected ? <EmptyState icon={<Headphones size={26} />} title="Выберите обращение" description="Просмотрите историю, примите обращение в работу и ответьте клиенту." /> : <><div className="operator-detail-header"><div><span className="section-eyebrow">ОБРАЩЕНИЕ ОПЕРАТОРА</span><h2>{selected.queue}</h2></div><button className="button button-secondary button-small" onClick={() => onOpen(selected.sessionId)}>Открыть диалог<ArrowRight size={14} /></button></div><div className="handoff-summary"><strong>Контекст передачи</strong><p>{selected.summary}</p><span>{selected.reason}</span></div>{error && <ErrorNotice message={error} onDismiss={() => setError(null)} />}{loading ? <div className="inline-loading"><Spinner label="Загружаем историю…" /></div> : <div className="operator-transcript">{detail?.turns.map(turn => <div key={turn.id}>{turn.mode !== "operator" && turn.userText && <p><strong>Клиент</strong>{turn.userText}</p>}<p><strong>{turn.mode === "operator" ? "Оператор" : "DIR ECHOES"}</strong>{turn.assistantText}</p></div>)}</div>}{selected.status !== "closed" ? <div className="operator-compose"><label htmlFor="operator-message">Ответ клиенту</label><textarea id="operator-message" value={message} onChange={e => setMessage(e.target.value)} maxLength={2000} placeholder="Напишите ответ. Он сохранится в разговоре клиента." disabled={busy} /><div>{selected.status === "waiting" && <button className="button button-secondary button-small" onClick={() => update("active", false)} disabled={busy}>Принять в работу</button>}<button className="button button-primary button-small" disabled={busy || !message.trim()} onClick={() => update("active", true)}>{busy ? <Spinner /> : <MessageSquare size={14} />}Отправить ответ</button><button className="button button-ghost button-small" disabled={busy} onClick={() => update("closed", !!message.trim())}><CircleCheck size={15} />Завершить обращение</button></div></div> : <div className="closed-note"><CircleCheck size={17} />Обращение завершено {dateTime(selected.updatedAt)}.</div>}</> }</div></div>}
  </section>;
}

    ...
    <div className="trace-scroll">
      {tab === "context" ? <>
        <section className="trace-section"><div className="trace-section-heading"><Layers3 size={15} /><h3>Состояние разговора</h3></div>{state ? <>
          <dl className="slot-list"><div><dt>Статус</dt><dd>{sessionStatus(state.status)}</dd></div><div><dt>Язык</dt><dd>{languageLabel(state.language, state.responseLanguages)}</dd></div><div><dt>Текущий сценарий</dt><dd>{active ? scenarioDisplayName(active.scenario_id, active.name) : "Не выбран"}</dd></div></dl>
        ...
        {trace.tone && <p className="tiny muted">Стиль ответа: {{ neutral: "нейтральный", calm: "спокойный", reassuring: "поддерживающий" }[trace.tone]}. Язык ответа: {languageLabel(trace.responseLanguage ?? trace.language, trace.responseLanguages)}.</p>}
